...
import flower.gallery.Bill.data.Billdetails;
import flower.gallery.Bill.data.orderDetails;
import flower.gallery.data.Order;
...
public class BillManager {
    ...
    public boolean updateOrderDetailsEntry(orderDetails entry){
        String addQuery = "insert into order_details (order_id, flower_id, quantity, unit_price) values (?,?,?,?)";
        PreparedStatement preparedStatement = null;
        int count = 0;
        try {
...
//          int price   = currentItem.getTotalForItem();
    ...
    public boolean updateQuantity(orderDetails entry){
        String updateQuery = "UPDATE inventory_flower set qty = qty - ?   where flower_id = ?";
        PreparedStatement preparedStatement = null;
        int count = 0;
        try {
    ...
    int getQtySingleItem(int bookId) {
        String query = "SELECT qty FROM inventory_flower where flower_id = ?";
        PreparedStatement preparedStatement = null;
        ResultSet resultSet;
        int qty = 0 ;
        try {
    ...
    public void jasperInvoice(int lastAddedOrder){
        ...
        try{
            InputStream path = getClass().getResourceAsStream("/flower/gallery/jasper/bill.jasper");
